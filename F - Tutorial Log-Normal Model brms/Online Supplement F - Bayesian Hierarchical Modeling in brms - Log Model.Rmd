---
title             : "Online Supplement F: Bayesian Hierarchical Modeling in brms - Log Model"
...
|    This online supplement provides a tutorial on log-normal Bayesian Hierarchical models (BHM) in `brms` [@R-brms_a; @R-brms_b]. In addition, it contains information about the Number Classification Task that could not be included in the paper. As `brms` runs on `Stan` [@carpenter2017stan], it is very similar to `rstan` [@R-rstan]. However, there are some differences that will be discussed in this tutorial. Only the fit of the full model will be shown as the fitting the others should follow logically from this tutorial. Technical details, such as explanations of statistical terms (e.g., $\hat{R}$), will not be discussed here but can be found in the paper. In concrete, the tutorial will cover the following:   
...
- For data structuring: *LaplacesDemon* [Version 16.1.4; @R-LaplacesDemon], *plyr* [Version 1.8.4; @R-plyr], *dplyr* [Version 0.8.3; @R-dplyr], *readr* [Version 1.3.1; @R-readr], *truncnorm* [Version 1.0.8; @truncnorm]
- To fit the model and for model comparison: *brms* [Version 2.13.0; @R-brms_a; @R-brms_b]
- For the visualization of the results: *lattice* [Version 0.20.38; @R-lattice], *ggplot2* [Version 3.3.2; @R-ggplot2], *Rmisc* [Version 1.5; @R-Rmisc], *devtools* [Version 2.2.1; @R-devtools], *gghalves* [Version 0.1.0; @R-gghalves], *bayesplot* [Version 1.7.1; @R-bayesplot], *gridExtra* [Version 2.3; @R-gridExtra], *ggridges* [Version 0.5.3; @ggridges], *ggbeeswarm* [Version 0.6.0; @ggbeeswarm], *tibble* [Version 3.0.2; @R-tibble]
...
|    The formula is an object that specifies the dependent variable as a function of the independent variables. The `~` (tilde) separates the dependent variable (`rt`) on the left side from the independent variables on the right side. The formula distinguishes between general effects and individual deviations. The first part of the equation on the predictor side of the formula shown below (i.e., `1 + side + dif1 + dif2 + dif3 + dif4`) represents the general effects. The `1` represents the intercept, in the case of the symbolic distance effect $\mu_{\gamma}$, `side`, and `dif1` to `dif4` represent the general effects of side and distance ($\mu_{\beta}$ and $\mu_{\delta}$). Within the round brackets individual deviations and grouping variables are specified. In this case the grouping variable (`ind`) indicates that each participant has an individual effect. In other cases effects might vary per item, block, or even the combination of participant and item. The double bar (`||`) indicates that the correlation between the parameters should not be modeled. When using a single vertical bar (`|`), correlations across all individual effects (intercept, side and distance) are modeled. This approach, however, is a bit more difficult to interpret and would also not correspond to the implemented `Stan` model. 
&nbsp;
...
for (i in 1:1000){  # with 10000 my computer cannot handle it 
  ...
  vargammapp <- rinvgamma(1, 13.8, 4.1)
  mubetapp <- rnorm(1, 0, 0.07)
  varbetapp <- rinvgamma(1, 13.8, 0.7)
  mudeltapp <- rnorm(1, 0, 0.07)
  vardeltapp <- rinvgamma(1, 13.8, 0.7)
  ...
  sigmapp <- rinvgamma(1, 13.8, 4.1)  # Only 1 because not individual specific? 
...
We have already estimated the model and saved it in the R object `brmslogmodelfit_final`. In case you do not want to wait until the model has been fitted but continue immediately with the rest of this tutorial, you can load this R object that contains the model estimations.  
&nbsp; 

\scriptsize 
```{r loadmodelfit, eval = FALSE}
logmodel_fit <- readRDS(file = "myPath/brmslogmodelfit_final.rds")  

# For model comparison - sample_prior was set to "yes" 
logmodel_fit_bf <- readRDS("myPath/brmslogmodelfit_BF_final.rds")  

```
\normalsize 

\scriptsize 
```{r loadmodelfits, show = FALSE}
logmodel_fit <- readRDS(file = "/Users/myrtheveenman/Documents/GitHub/multilevel-tutorial-paper/R objects/brms/non-linear/brmmodel8log13012022.rds")  

# For model comparison - sample_prior was set to "yes" 
logmodel_fit_bf <- readRDS("/Users/myrtheveenman/Documents/GitHub/multilevel-tutorial-paper/R objects/brms/BF/brmmodel8log26052022_bf.rds")  
...
tr2 <- traceplotfunc(parameter = "sd_sub1__side", title = expression(~sigma[~beta]))
tr2 <- tr2 + theme(legend.position = "none")
tr3 <- traceplotfunc(parameter = "sd_sub1__dif1", title = expression(~sigma[~delta[~7]]))
tr3 <- tr3 + theme(legend.position = "none")
tr4 <- traceplotfunc(parameter = "sd_sub1__dif2", title = expression(~sigma[~delta[~6]]))
tr4 <- tr4 + theme(legend.position = "none")
tr5 <- traceplotfunc(parameter = "sd_sub1__dif3", title = expression(~sigma[~delta[~4]]))
tr5 <- tr5 + theme(legend.position = "none")
tr6 <- traceplotfunc(parameter = "sd_sub1__dif4", title = expression(~sigma[~delta[~3]]))
legendtr <- cowplot::get_legend(tr6)
tr6 <- tr6 + theme(legend.position = "none")
...
\scriptsize 
```{r neffexamp5}
neffbrmsratio_value <- mcmc_plot(logmodel_fit, type = "neff_hist")$data$value   # save neff values, these are the values of the ration Neff/N, N representing the total number of iterations
...
fixedneffbrms2 <- c(neffbrmsratio_comb[grep("b_Intercept", neffbrmsratio_comb$parameter),]$value[1],
                      neffbrmsratio_comb[grep("b_side", neffbrmsratio_comb$parameter),]$value[1],
                      neffbrmsratio_comb[grep("b_dif1", neffbrmsratio_comb$parameter),]$value[1],
                      neffbrmsratio_comb[grep("b_dif2", neffbrmsratio_comb$parameter),]$value[1],
                      neffbrmsratio_comb[grep("b_dif3", neffbrmsratio_comb$parameter),]$value[1],
                      neffbrmsratio_comb[grep("b_dif4", neffbrmsratio_comb$parameter),]$value[1])
...
neffbrmsratio3 <- c(neffbrmsratio_comb[grep(",Intercept]", neffbrmsratio_comb$parameter),]$value,
...
# Beta 
brmsbetar <- coef(logmodel_fit)$sub1[,,"side"][,c("Estimate", "Q2.5", "Q97.5")]
brmsbetar <- as.data.frame(brmsbetar)
fixbremsbetar <- fixef(logmodel_fit)["side","Estimate"]
...
# Delta 1 
brmsdel1r <- coef(logmodel_fit)$sub1[,,"dif1"][,c("Estimate", "Q2.5", "Q97.5")]
brmsdel1r <- as.data.frame(brmsdel1r)
fixbremsdif2r <- fixef(logmodel_fit)["dif1","Estimate"]
...
# Delta 2 
brmsdel2r <- coef(logmodel_fit)$sub1[,,"dif2"][,c("Estimate", "Q2.5", "Q97.5")]
brmsdel2r <- as.data.frame(brmsdel2r)
fixbremsdif22r <- fixef(logmodel_fit)["dif2","Estimate"]
...
# Delta 3 
brmsdel3r <- coef(logmodel_fit)$sub1[,,"dif3"][,c("Estimate", "Q2.5", "Q97.5")]
brmsdel3r <- as.data.frame(brmsdel3r)
fixbremsdif3r <- fixef(logmodel_fit)["dif3","Estimate"]
...
# Delta 4 
brmsdel4r <- coef(logmodel_fit)$sub1[,,"dif4"][,c("Estimate", "Q2.5", "Q97.5")]
brmsdel4r <- as.data.frame(brmsdel4r)
fixbremsdif4r <- fixef(logmodel_fit)["dif4","Estimate"]
...
brmmodel8bf <- readRDS("myPath/brmslogmodelfit_BF_final.rds")
...
b1brms <- hypothesis(x = logmodel_fit_bf, hypothesis = "side = 0")  
plotsdratiob1 <- plot(b1brms, plot = F, theme = theme_get())[[1]]
sdrbetplot <- savdicratioplot(plotsdratiob1) 
sdrbetplot <- sdrbetplot + ggtitle(expression(~mu[~beta])) + xlim(-0.1,0.1) + geom_point(aes(x = 0, y = 30), color = "darkorange", size = 1)
sdrbetplot <- sdrbetplot + 
  geom_segment(aes(x = 0, y = 5.7, xend = 0, yend = 30), size=0.1, color = "black", linetype = "dashed") + 
  annotate("text", x = c(0.03, 0.037), xend = 0.03, y = 10, yend = 10.4, label = c(expression(paste("BF"["01"], " = ")), round(b1brms$hypothesis$Evid.Ratio, 2)), size = 2) 

# Dif 1 
h1brms <- hypothesis(logmodel_fit_bf, "dif1 = 0")  # Evid.Ratio is Bayes factor BF01, have to set sample_prior = TRUE when fitting model 
...
sdrdif1plot <- sdrdif1plot + 
  ...
  geom_point(aes(x = 0, y = 1.7), color = "darkorange", size = 1) + 
  geom_segment(aes(x = 0, y = 5.7, xend = 0, yend = 2), size=0.1, color = "black", linetype = "dashed") + 
  annotate("text", x = c(-0.02, -0.012), xend = 0.03, y = 10, yend = 10.4, label = c(expression(paste("BF"["01"], " = ")), round(h1brms$hypothesis$Evid.Ratio, 2)), size = 2) + 
  ylab("")

# Dif 2 
h2brms <- hypothesis(logmodel_fit_bf, "dif2 = 0")  # Evid.Ratio is Bayes factor BF01, have to set sample_prior = TRUE when fitting model 
...
sdrdif2plot <- sdrdif2plot + 
  ...
  geom_segment(aes(x = 0, y = 5.7, xend = 0, yend = 0), size=0.1, color = "black", linetype = "dashed") + 
  annotate("text", x = c(0.0, 0.005), xend = 0.03, y = 10, yend = 10.4, label = c(expression(paste("BF"["01"], " = ")), round(h2brms$hypothesis$Evid.Ratio, 2)), size = 2) 

# Dif 3 
h3brms <- hypothesis(logmodel_fit_bf, "dif3 = 0")  # Evid.Ratio is Bayes factor BF01, have to set sample_prior = TRUE when fitting model 
...
sdrdif3plot <- sdrdif3plot + 
  ...
  geom_point(aes(x = 0, y = 0), color = "darkorange", size = 1) + 
  geom_segment(aes(x = 0, y = 5.7, xend = 0, yend = 0), size=0.1, color = "black", linetype = "dashed") + 
  annotate("text", x = c(0.0, 0.005), xend = 0.03, y = 10, yend = 10.4, label = c(expression(paste("BF"["01"], " = ")), round(h3brms$hypothesis$Evid.Ratio, 2)), size = 2) + 
  ylab("")

# Dif 4 
h4brms <- hypothesis(logmodel_fit_bf, "dif4 = 0")  # Evid.Ratio is Bayes factor BF01, have to set sample_prior = TRUE when fitting model 
...
sdrdif4plot2 <- ggplot(xypriorsd, aes(x, y, color = Type, fill = Type)) + 
  geom_line() + 
  geom_area(position = "identity", alpha = 0.3) + 
  geom_point(aes(x = 0 , y = 5.6), size = 1) + 
  geom_point(aes(x = 0, y = 0), color = "darkorange", size = 1) + 
  xlim(-0.1, 0.1) + 
  ylab("Density") + 
  xlab("Value") + 
  ...
  scale_fill_manual(values = c("darkorange", "darkgreen")) + 
  scale_color_manual(values = c("darkorange", "darkgreen")) + 
  geom_segment(aes(x = 0, y = 5, xend = 0, yend = 0), size=0.1, color = "black", linetype = "dashed") + 
  annotate("text", x = c(-0.01, -0.005), xend = 0.03, y = 10, yend = 10.4, label = c(expression(paste("BF"["01"], " = ")), round(h4brms$hypothesis$Evid.Ratio, 2)), size = 2) + 
