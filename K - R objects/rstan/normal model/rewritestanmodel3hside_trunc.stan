...
model {
  ...
  // Hyperpriors.
  target += normal_lpdf(mu| a, b)-normal_lccdf(0.0 | a, sqrt(b));             // mean of intercept (gamma), b = standard deviation , sqrt(b) is sd but since 1 not necessary to sqrt()
  ...
  //betaii 
  target += normal_lpdf(y | gammai[group_inds] + side .* betai[group_inds], sqrt(sigma)); // does not work
