...
|    This online supplement provides a tutorial on Bayesian Hierarchical models (BHM) in `rstan` [@R-rstan]. In addition, it contains information about the Number Classification Task that could not be included in the paper. Only the fit of the full model will be shown as the fitting the others should follow logically from this tutorial. Technical details, such as explanations of statistical terms (e.g., $\hat{R}$), will not be discussed here but can be found in the paper. This tutorial will cover the following:   
