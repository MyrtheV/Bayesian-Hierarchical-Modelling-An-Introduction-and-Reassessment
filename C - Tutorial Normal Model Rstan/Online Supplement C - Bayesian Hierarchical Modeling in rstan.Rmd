---
title             : "Online Supplement C: Bayesian Hierarchical Modeling in rstan"
author            : " " 
...
|    This online supplement provides a tutorial on Bayesian Hierarchical models (BHM) in `rstan` [@R-rstan]. In addition, it contains information about the digit classification task that could not be included in the paper. Only the fit of the full model will be shown as the fitting the others should follow logically from this tutorial. Technical details, such as explanations of statistical terms (e.g., $\hat{R}$), will not be discussed here but can be found in the paper. This tutorial will cover the following:   

1. Required packages 
2. Input  
  2.1 Stan file   
  2.2 Data  
  2.3 Individual Effect Correlation 
3. Fit model 
4. Output   
  4.1 General Effects   
...
- To fit the model: *rstan* [Version 2.21.3; @R-rstan]
- For the visualization of the results: *lattice* [Version 0.20.44; @R-lattice], *ggplot2* [Version 3.3.2; @R-ggplot2], *Rmisc* [Version 1.5; @R-Rmisc], *devtools* [Version 2.4.2; @R-devtools], *gghalves* [Version 0.1.1.9000; @R-gghalves], *bayesplot* [Version 1.8.1; @R-bayesplot], *brms* [Version 2.16.3; @R-brms_a; @R-brms_b], *gridExtra* [Version 2.3; @R-gridExtra], *ggridges* [Version 0.5.3; @R-ggridges], *ggbeeswarm* [Version 0.6.0; @R-ggbeeswarm], *tibble* [Version 3.1.6; @R-tibble]
- For model comparison: *bridgesamping* [Version 1.1.2; @R-bridgesampling]
...
|    To fit a BHM in *rstan*, we use the *stan* function, as shown below. 
&nbsp; 
...
|  The stan file `myModel.stan` is used to fit the full model and can be found under *K - R objects > rstan > normal model*. It contains descriptions for every line of code. The stan file is divided in three sections (in our case): 
...
\textcolor{blue}{Up until now, we discussed the normal model from the manuscript. This model assumes that there is no correlation between the individual effects. As stated in the manuscript, it is also possible to allow for the estimation of the correlation between individual effects. This means, for instance, that we would expect that the estimates for the individual effects} $\delta_{7,i}$ and $\delta_{3,i}$ \textcolor{blue}{are related. To allow for correlation between individual effects, we would have to adjust the formula in the} `stan` \textcolor{blue}{file and the priors. For the model with correlation terms, we use the file} `myModel_cor.stan` \textcolor{blue}{that can be found in the folder} *K - R objects > rstan > normal model > correlation*. \textcolor{blue}{Notice that we apply a different conceptualization of the model using matrix notation. The effects are called} `beta` \textcolor{blue}{and the individual effects are called} `beta_p`. \textcolor{blue}{Because we are using matrices, the effects are stored a bit differently than for the other model. For example,} `beta[3]` \textcolor{blue}{represents the general digit effect} $\delta_{7}$, \textcolor{blue}{while} `beta_p[1,3]` \textcolor{blue}{represents the digit effect for participant 1,} $\delta_{7,1}$.} 

\textcolor{blue}{When including correlation between individual effects, we are estimating more parameters and have to specify priors for these parameters as well. In specific, we set a prior on the correlation matrices. These priors will be transformed for efficiency and set on so called Cholesky factors of the matrices by the following line of code} `Lcorr ~ lkj_corr_cholesky(1)` \textcolor{blue}{[for more information on Cholesky factors see} @stan2018a; \textcolor{blue}{section 1.13]. }
...
library(rstan) 
model_fit <- stan(file = "myPath/myModel.stan", 
                  data = myData_list, 
                  iter = 4000, 
                  chains = 4,
                  warmup = 1000, 
...
\textcolor{blue}{To estimate the model with the correlation between individual effects, note that one would need to adjust the} `file` \textcolor{blue}{argument to equal} `myModel_cor.stan`.  

\textcolor{blue}{Back to our model without the correlation term form the manuscript.} We have already fitted the model and saved it in the R object `model_fit`. In case you do not want to wait until the model has been fitted but continue immediately with the rest of this tutorial, you can load this R object that contains the model estimates.  
&nbsp; 

\scriptsize 
```{r loadmodelfit, eval = FALSE}
model_fit <- readRDS(file = "myPath/rstan_model_fit.rds")
```
\normalsize 
```{r loadmodelfit2, echo = FALSE}
setwd('..')
model_fit <- readRDS(file = "K - R objects/rstan/normal model/hier_modelc_adj_07012022.rds")
```
...
trdif2rstan <- plot(model_fit,     # Model fit 
...
trdif2rstanwarmup <- plot(model_fit,    # Model fit 
                          plotfun = "trace",  # Specify that you want trace plots 
                          pars = c("mu4"),    # Select parameter
...
plot(model_fit, plotfun= "rhat")

# Plot frequency of the ratio 
plot(model_fit, plotfun = "ess")
```
\normalsize 
...
rhatrstan <- summary(model_fit)$summary[,"Rhat"]

# Obtain number of effective samples for every paramter from model fit 
neffrstan <- summary(model_fit)$summary[,"n_eff"]
...
postc <- as.array(model_fit)  # change structure of model fit 
...
gamstan <- as.data.frame(summary(model_fit, 
                                 pars = c( gamnamesstan))$summary)

# Extract the general effect estimate of the parameter 
gamstanmu <- as.data.frame(summary(model_fit, 
                                   pars = c("mu"))$summary)
...
betstan <- as.data.frame(summary(model_fit, pars = c( betnamesstan))$summary)
betstanmu <- as.data.frame(summary(model_fit, pars = c("mu2"))$summary)
...
delt1stan <- as.data.frame(summary(model_fit, pars = c( del1namesstan))$summary)
del1stanmu <- as.data.frame(summary(model_fit, pars = c("mu3"))$summary)
...
delt2stan <- as.data.frame(summary(model_fit, pars = c( del2namesstan))$summary)
del2stanmu <- as.data.frame(summary(model_fit, pars = c("mu4"))$summary)
...
delt3stan <- as.data.frame(summary(model_fit, pars = c( del3namesstan))$summary)
del3stanmu <- as.data.frame(summary(model_fit, pars = c("mu5"))$summary)
...
delt4stan <- as.data.frame(summary(model_fit, pars = c( del4namesstan))$summary)
del4stanmu <- as.data.frame(summary(model_fit, pars = c("mu6"))$summary)
...
hier_modeld <- model_fit  # model fit 
...
plotmodelest_m7 <- ggplot(data=datafr_m7, aes(y=mean)) +
  ...
  ggtitle('') +
  theme_classic() + 
  coord_cartesian(xlim = c(0.75, 5), ylim = c(-0.1, 0.15))
...
knitr::include_graphics("Fig12_modelestimatefigure2_onlyrstancor_v2.png")
```
...
|    First, we fit all the models without correlation between individual effects (except for the full model that we already fitted. 
&nbsp; 
...
## Null model 
BFnullmodel <- stan(file = "myPath/rewritestanmodel3h0.stan", 
                    data = myData_list, 
                    iter = 4000, 
                    chains = 4,  
                    warmup = 1000, 
...
## Side model 
BFsidemodel <- stan(file = "myPath/rewritestanmodel3hside.stan", 
                       data = myData_list, 
                       iter = 4000, 
                       chains = 4, 
                       warmup = 1000, 
...
## Digit model 
BFdigitmodel <- stan(file = "myPath/rewritestanmodel3hdigit.stan", 
                         data = myData_list, 
                         iter = 4000, 
                         chains = 4, 
                         warmup = 1000, 
...
# Bridge sampling 
# library(bridgesampling)
bridge_Hnull <- bridge_sampler(BFnullmodel)
bridge_Hside <- bridge_sampler(BFsidemodel)
bridge_Hdigit <- bridge_sampler(BFdigitmodel)
bridge_Hfull <- bridge_sampler(model_fit)  # in case you just fitted the model yourself 
...
new_mod <- stan(file = "myPath/rewritestanmodel3_julia.stan", 
                data = myData_list, 
                chains = 0)

### Now do the bridgesampling 
bridge_Hfull <- bridge_sampler(model_fit, new_mod)
```
\normalsize 
...
bridgesampling::bf(bridge_Hnull, bridge_Hfull)   # Null against full 

# Obtain BF (in this way evidence in favor of the full hypothesis)
bridgesampling::bf(bridge_Hfull, bridge_Hnull)   # Full against null
bridgesampling::bf(bridge_Hfull, bridge_Hside)   # Full against side 
bridgesampling::bf(bridge_Hfull, bridge_Hdigit)  # Full against digit 
```
\normalsize 
