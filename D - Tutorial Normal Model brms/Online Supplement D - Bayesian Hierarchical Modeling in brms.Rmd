...
The difference is that instead of using the double bar `||`, we are now using a single bar `|` before `ind`, indicating that we allow individual effects to correlate. 

This means that we are estimating more parameters and therefore we have to specify priors on these parameters as well. In specific, we can set a prior on the correlation matrix called `class = "cor"`. These priors will be transformed for efficiency and set on so called Cholesky factors of the matrices [for more information see @stan2018a; section 1.13]. In our example, we set a prior on the Cholesky factors directly, by defining `class = "L"`, shown in the last line of the code block: 
...
To estimate the model with the correlation between individual effects, note that one would need to adjust `formula` and `priorsmodel` to the objects where these are saved in (i.e., `bform_cor` and `priorsmodel_cor`). 
